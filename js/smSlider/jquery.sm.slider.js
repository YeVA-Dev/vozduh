...
(function($) {
	$.fn.smSlider = function(options) {
		...
		return this.each(function() {
			...
			var slideMove = function(toIndex, direction) {
				if (clickable) {
					...
					if (options.transition != 'fader') {
						$smSliderInner.children(
								'.' + options.children + '.'
										+ options.activeClass).stop().animate({
							'left' : currPos
						}, {
						...
						$smSlide.eq(toIndex).css('left', nextPos).addClass(
								options.activeClass).stop().animate({
							'left' : 0
						}, {
							duration : options.duration,
						...
						$smSliderInner.children(
								'.' + options.children + '.'
										+ options.activeClass).stop().fadeOut({
							duration : options.duration,
							easing : options.easing,
							complete : function() {
								$(this).removeClass(options.activeClass);
			...
			var smSlideHeight = $smSlider.height() - options.heightCtrl;
			if (options.flexible) {
			...
			;
			var $smSliderInner = $smSlider.children('ul').addClass(
					options.innerBlock);
			$smSliderInner.css({
			...
			var $smSlide = $smSliderInner.children('li').addClass(
					options.children);
			...
			if (smSlideSizer > 1) {
				var $customCtrl=null;
				if (options.autoArr || options.pagination) {
					var $customCtrl = $('<div/>').addClass('sm_ctrl');
					$customCtrl.appendTo($smSlider);
				}
				var $elemCtrl = $('<i/>');
				if (options.autoArr) {
					var $smPrev = $elemCtrl.clone().addClass(options.prev);
					var $smNext = $elemCtrl.clone().addClass(options.next);
					$smPrev.appendTo($customCtrl);
					$smNext.appendTo($customCtrl);
				} else {
				...
				if (options.pagination) {
					...
					if (options.typeCtrl != 'numeric') {
						...
						for (i = 0; i < smSlideSizer; i++) {
							$smNavItem.clone().data({
								'index' : i
							}).addClass('sm_nav-item').text(i + 1).appendTo(
									$smNav);
						}
					}
